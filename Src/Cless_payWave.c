...
void Cless_payWave_DebugActivation (int bActivate) {
	...
	if (pSharedStructure != NULL) {
		...
		if (nResult != STATUS_SHARED_EXCHANGE_OK) {
			GTL_Traces_TraceDebug("Cless_payWave_DebugActivation : Unable to add TAG_KERNEL_DEBUG_ACTIVATION (nResult = %02x)", nResult);
		} else {
			nResult = payWave_DebugManagement(pSharedStructure);

			if (nResult != KERNEL_STATUS_OK)
			{
				GTL_Traces_TraceDebug("Cless_payWave_DebugActivation : Error occurred during payWave Debug activation (nResult = %02x)", nResult);
			}
		}
...
int Cless_payWave_CustomiseStep (T_SHARED_DATA_STRUCT * pSharedData, const unsigned char ucCustomisationStep)
{
	...
	case STEP_PAYWAVE_MSD_REMOVE_CARD:
		mapPutByte(appClessMagMode, 1);//MSTRIPE active
	...
	case STEP_PAYWAVE_QVSDC_REMOVE_CARD:
		mapPutByte(appClessMagMode, 0);//MCHIP active
		perflog("MG\tpW_CUST\tSTEP_PAYWAVE_xxxx_REMOVE_CARD");
		HelperRemoveCardSequence(pSharedData);
		GTL_SharedExchange_ClearEx (pSharedData, FALSE);
		nResult = KERNEL_STATUS_CONTINUE;
	...
	case STEP_PAYWAVE_QVSDC_GET_CERTIFICATE: {
		...
		if (!Cless_Parameters_GetCaKeyData (pTreeCurrentParam, ucCapkIndex, ucRid, pSharedData)) {
			GTL_Traces_TraceDebug ("Cless_payWave_CustomiseStep : Cless_Parameters_GetCaKeyData failed");
		}
	...
	case STEP_PAYWAVE_QVSDC_BLACK_LIST_CONTROL:
	{
		...
		if (Cless_BlackListIsPan((int)ulLgPan, pPan, (int)(*pPanSeqNumber)) == TRUE)
		{
			...
			if (cr != STATUS_SHARED_EXCHANGE_OK)
			{
				GTL_Traces_TraceDebug("Cless_payWave_CustomiseStep An error occurred when adding TAG_KERNEL_PAN_IN_BLACK_LIST in the shared buffer (cr = %02x)", cr);
				GTL_SharedExchange_ClearEx (pSharedData, FALSE);
				break;
			}
			nCardInBlackList = 1;
	...
	default:
		...
		GTL_Traces_TraceDebug ("Cless_payWave_CustomiseStep : Step to customise (unknown) = %02x", ucCustomisationStep);
		break;
	}
...
static int __Cless_payWave_CheckISPConditions(T_SHARED_DATA_STRUCT * pSharedStructure) {
	...
	if (cr == STATUS_SHARED_EXCHANGE_OK) {
		if ((pReadValue[2] & 0x80) == 0) {	// Sorry, TTQ not configurated for ISP management => end
...
static word Cless_payWave_DoTransactionTask (void) {
	do {
		perflog("MG\tpW_CUST\tpayWave_DoTransaction");

		mapPut(appCardName, "VISA", 4);

		g_DoTransactionCr = payWave_DoTransaction(g_pDataStruct);

		perflog("MG\tpW_CUST\tEnd payWave_DoTransaction");
		perflog("MG\tpW_CUST\tCless_payWave_DoTransactionTask SignalEvent");
...
static int __Cless_payWave_StartDoTransaction (T_SHARED_DATA_STRUCT * pDataStruct) {
	perflog("MG\tpW_CUST\t__Cless_payWave_StartDoTransaction");
	...
	if (g_DoTransactionTask == NULL) {
		...
		g_DoTransactionTask = Telium_Fork (&Cless_payWave_DoTransactionTask, NULL, -1);
		...
		if (g_DoTransactionTask != NULL) {
			...
			perflog("MG\tpW_CUST\tExit __Cless_payWave_StartDoTransaction");
			return OK;
		} else {
		...
		g_pDataStruct = pDataStruct;
		perflog("MG\tpW_CUST\t__Cless_payWave_StartDoTransaction Telium_SignalEvent");
		Telium_SignalEvent(g_DoTransactionTaskIdTask, E_USER_EVENT_START);
		perflog("MG\tpW_CUST\tExit __Cless_payWave_StartDoTransaction Telium_SignalEvent");
		return OK;
	}
}
#endif
...
int Cless_payWave_PerformTransaction (T_SHARED_DATA_STRUCT * pDataStruct) {
	...
	perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction");
	...
	perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 01");

	// Indicate payWave kernel is going to be used (for customisation purposes)
	Cless_Customisation_SetUsedPaymentScheme (CLESS_SAMPLE_CUST_PAYWAVE);
	perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 02");

	if (!__Cless_payWave_AddpayWaveSpecificData(pDataStruct)) {
		GTL_Traces_TraceDebug("Cless_payWave_PerformTransaction : __Cless_payWave_AddpayWaveSpecificData error\n");
	} else {
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 03");
		...
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 04");
		...
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 05");

		// 
#ifndef __TELIUM3__
		Cless_Scan_TransacOpenDrivers();
#endif
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 06");
...
#endif
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 07");
		...
		perflog("MG\tpW_CUST\tCless_payWave_PerformTransaction 08");
		...
		perflog("MG\tpW_CUST\tEnd payWave_DoTransaction");
#else
		cr = __Cless_payWave_StartDoTransaction(pDataStruct);
		if (cr == OK) {
		...
		if (cr & KERNEL_STATUS_STOPPED_BY_APPLICATION_MASK) {	// If mask has been set
			...
			case CAM0:			// Chip card
				if (Cless_Scan_TransacCam0(ulAmount, nCurrencyCode) == FALSE)
					GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction Pb Transac Cam 0");

				break;

			case SWIPE2:		// Swipe
				if (Cless_Scan_TransacSwipe2(ulAmount, nCurrencyCode) == FALSE)
					GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction Pb Transac Swipe 2");
			...
			case KERNEL_STATUS_OFFLINE_APPROVED:
				...
				if (bLoopMode == 0) {
					...
					//					Cless_Receipt_PrintTransaction(pDataStruct, BATCH_TICKET_VISA, APPROVED_TICKED, ucSignature, nCardHolderLang, WITH_AOSA);
				}
			...
			case KERNEL_STATUS_OFFLINE_DECLINED:
				...
				if (bLoopMode == 0) {
					...
					//					Cless_Receipt_PrintTransaction(pDataStruct, BATCH_TICKET_VISA, DECLINED_TICKED, WITHOUT_SIGNATURE, nCardHolderLang, WITH_AOSA);
				}
			...
			case KERNEL_STATUS_ONLINE_AUTHORISATION:
				...
				if (ucOnLinePinRequested) { // If OnLine Pin Requested
					...
					if (GTL_SharedExchange_FindNext (pDataStruct, &nPosition, TAG_EMV_APPLI_PAN, &ulLgPan, &pPan) != STATUS_SHARED_EXCHANGE_OK) {
						GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction : Missing PAN for Online PIN");
					...
					if (GTL_SharedExchange_FindNext (pDataStruct, &nPosition, TAG_EMV_AMOUNT_AUTH_NUM, &ulLgAmount, &pAmount) != STATUS_SHARED_EXCHANGE_OK) {
						GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction : Missing Amount auth for Online PIN");
					...
					if (!bOnlinePinError) {
						...
						if (cr_pin_online == INPUT_PIN_ON) {
							...
							if (cr != STATUS_SHARED_EXCHANGE_OK) {
								GTL_Traces_TraceDebug("Cless_payWave_PerformTransaction : Unable to add TAG_SAMPLE_ENCIPHERED_PIN_CODE in shared buffer (cr=%02x)", cr);
						...
						if (GTL_SharedExchange_FindNext(pDataStruct, &nPosition, TAG_EMV_AUTHORISATION_RESPONSE_CODE, &ulReadLength, &pReadValue) == STATUS_SHARED_EXCHANGE_OK) {
							if ((pReadValue[0] == 0x30) && (pReadValue[1] == 0x30)) {
								...
								if (bLoopMode == 0) {
									//								if ((Cless_DumpData_DumpGetOutputId() != CUSTOM_OUTPUT_NONE) && ((ucSignature) || (HelperQuestionYesNo (&tDisplayMsg, 30, merchLang)))) {
									//									if (nAfterTxnExecuted)
									//										Cless_Receipt_PrintTransaction(pDataStruct, BATCH_TICKET_VISA, APPROVED_TICKED, ucSignature, nCardHolderLang, WITHOUT_AOSA);
									//									else
									//										Cless_Receipt_PrintTransaction(pDataStruct, BATCH_TICKET_VISA, APPROVED_TICKED, ucSignature, nCardHolderLang, WITH_AOSA);
									//								}
								}
...
#endif
					GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction : An error occurred for PIN Entry");
				}
			...
			default:
				GTL_Traces_TraceDebug ("Cless_payWave_PerformTransaction result = %02x", cr);
...
void Cless_payWave_AidRelatedData(T_SHARED_DATA_STRUCT * pSharedStruct, T_SHARED_DATA_STRUCT * pAidParameters) {
	// Local variables
	int cr;
	...
	if (pAidReadValueElement[0] & 0x20) {     // 		=> CVM required and Online Cryptogram required bits taken from Cless_debit_Aid parameters
		...
		if (ulTransactionAmount == 0) {
			...
			if (ulTransactionAmount >= ulTransactionLimit) {
				...
				Cless_payWave_SetQVSDCNotAllowedForAmount(TRUE);
				{
					unsigned char ucQvsdcNotAllowed = 1;
					// Delete the Contactless magnetic stripe (MSD) supported and Contactless qVSDC supported
					pAidReadValueElement[0] &= 0x5F;
...
//!     - FALSE else.
int Cless_payWave_IsQVSDCNotAllowedForAmount(void) {
...
void Cless_payWave_SetQVSDCNotAllowedForAmount(int nAmountGreaterThanTransactionLimit) {
